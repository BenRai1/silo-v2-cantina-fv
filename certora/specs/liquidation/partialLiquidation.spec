...
//------------------------------- RULES TEST START ----------------------------------
    ...
    // ---------------other user	
        // liquidationCall() no balances for other user are changed //@audit-issue still to write
    ...
    // ---------------msg.sender	
        // liquidationCall() msg.sender: underlyingTokenBalanceSender is reduced by repayDebtAssets //@audit-issue no summary of saveTransfer
        ...
        rule msgSenderNeverPaysMoreThanMaxDebtToCover(env e){
            ...
            //asserts
            assert sentTransferredAssetsSenderAfter <= maxDebtToCover;
        }
    ...
    // ---------------general	
        ...
        rule hookReceiverNeverOwnsCollateralShares(env e){
            ...
            //redeem
            uint256 redeemedAssetsProtectedCurrentContractAfter = redeemedAssets[collateralConfig.protectedShareToken][false][currentContract];
